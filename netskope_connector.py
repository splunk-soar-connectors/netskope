...
class NetskopeConnector(BaseConnector):
    ...
    def _process_empty_response(response, action_result):
        ...
        if response.status_code >= 200 and response.status_code < 300:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, ('Empty response and no information in the header. Status Code: {0}').format(response.status_code)),
                          None)
        ...
        try:
            ...
            if config.get(NETSKOPE_CONFIG_SCIM_URL) and config.get(NETSKOPE_CONFIG_SCIM_KEY):
                try:
                    ...
                    return RetVal(action_result.set_status(phantom.APP_ERROR, ('Error while encoding server URL: {0}').format(error_msg)), resp_json)
            else:
                self.debug_print("Please configure both 'SCIM Server URL' and 'SCIM Token' in asset settings to execute this action")
                return RetVal(action_result.set_status(phantom.APP_ERROR, "Please configure both 'SCIM Server URL' and 'SCIM Token' in asset settings to execute this action"),
                              resp_json)
            ...
            self._log.info(('action=make_scim_rest method={0} url={1} params={2} function={3}').format(method, url, json.dumps(params), request_func))
    ...
    def _make_rest_call(self, endpoint, action_result, method='get', params=None, data=None, timeout=None):
        ...
        if action in v2_supported_actions_list and not (self._v2_api_key or self._api_key):
            return RetVal(action_result.set_status(phantom.APP_ERROR, NETSKOPE_MISSING_BOTH_API_KEYS_ERR),
                          None)
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, 'Error while initializing server URL and basic connection parameters from the asset configuration'),
                          resp_json)
    ...
    def _handle_test_connectivity(self, param):
        ...
        if self._v2_api_key:
            ...
            ret_val, _ = self._make_rest_call(endpoint=('{0}/{1}').format(NETSKOPE_V2_EVENT_ENDPOINT, NETSKOPE_PAGE_EVENT),
    ...
    def _handle_get_file(self, param):
        ...
        try:
            ...
            for vault_file_item in vault_file_list:
                ...
                success, message, vault_add_file_dict = vault.vault_add(container=self.get_container_id(), file_location=temp_file_path, file_name=file_name)
    ...
    def _get_file_and_profile_details(self, action_result, file_param, profile_param):
        ...
        if phantom.is_fail(request_status):
            ...
            for item in request_response['data']['quarantined']:
                ...
                if item.get('quarantine_profile_id', '').lower() == profile_param.lower() or item.get('quarantine_profile_name', '').lower() == profile_param.lower():  # noqa: E501
    ...
    def _handle_run_query(self, param):
        ...
        ret_val, page_event_list = self._get_events(
            endpoint=page_event_endpoint if self._v2_api_key else NETSKOPE_EVENTS_ENDPOINT,
        ...
        ret_val, application_event_list = self._get_events(
            endpoint=application_event_endpoint if self._v2_api_key else NETSKOPE_EVENTS_ENDPOINT,
            action_result=action_result, params=params)

        if phantom.is_fail(ret_val):
            return action_result.set_status(phantom.APP_ERROR, ("Failed to get 'application' events. Error: {0}").format(action_result.get_message()))
    ...
    def _handle_on_poll(self, param):
        ...
        response_status, alerts_list = self._get_events(endpoint=alert_event_endpoint if self._v2_api_key else NETSKOPE_ON_POLL_ENDPOINT,
                                                        action_result=action_result, params=request_params, max_limit=container_count)
    ...
    def _create_container(self, alert):
        ...
        container_dict['name'] = ('{alert_name}-{id}-{type}').format(alert_name=alert['alert_name'], id=alert.get('_id', ('unk-{0}').format(uuid.uuid4())),
        ...
        possible_tags = {'alert_type': alert.get('alert_type'), 'category': alert.get('category'),
                         'activity': alert.get('activity'), 'type': alert.get('type')}
    ...
    def _create_artifacts(self, alert, container_id):
        ...
        artifacts_mapping = {'IP Artifact': {'managementID': ('managementID', []),
                                             'nsdeviceuid': ('nsdeviceuid', []),
                                             'Source IP': ('srcip', ['ip']),
                                             'Destination IP': ('dstip', ['ip'])},
                             'Email Artifact': {'managementID': ('managementID', []),
                                                'nsdeviceuid': ('nsdeviceuid', []),
                                                'Email': ('user', ['email']),
                                                'Source Email': ('from_user', ['email'])},
                             'URL Artifact': {'managementID': ('managementID', []),
                                              'nsdeviceuid': ('nsdeviceuid', []),
                                              'url': ('url', ['url'])},
                             'User Artifact': {'managementID': ('managementID', []),
                                               'nsdeviceuid': ('nsdeviceuid', []),
                                               'userkey': ('userkey', ['user name']),
                                               'user': ('user', ['user name']),
                                               'from_user': ('from_user', ['user name'])}}

        specific_alert_mapping = {'malsite': {'managementID': ('managementID', []),
                                              'nsdeviceuid': ('nsdeviceuid', []),
                                  ...
                                  'malware': {'managementID': ('managementID', []),
                                              'nsdeviceuid': ('nsdeviceuid', []),
                                              'alert_type': ('alert_type', []),
                                              'dstip': ('dstip', ['ip', 'ipv4']),
                                  ...
                                  'dlp': {'managementID': ('managementID', []),
                                          ...
                                          'browser': ('browser', []),
                                          'appcategory': ('appcategory', ['category'])},
                                  'anomaly': {'managementID': ('managementID', []),
                                              'nsdeviceuid': ('nsdeviceuid', []),
                                              'alert_type': ('alert_type', []),
                                  ...
                                  'compromised credential': {'managementID': ('managementID', []),
                                                             'nsdeviceuid': ('nsdeviceuid', []),
                                                             'alert_type': ('alert_type', []),
                                                             ...
                                                             'count': ('count', []),
                                                             'alert_name': ('alert_name', [])},
                                  'legal hold': {'managementID': ('managementID', []),
                                                 'nsdeviceuid': ('nsdeviceuid', []),
                                                 'alert_type': ('alert_type', []),
                                                 ...
                                                 'browser': ('browser', []),
                                                 'appcategory': ('appcategory', ['category'])},
                                  'policy': {'managementID': ('managementID', []),
                                             'nsdeviceuid': ('nsdeviceuid', []),
                                             'alert_type': ('alert_type', []),
                                             'dst_location': ('dst_location', []),
                                             'app': ('app', []),
                                             ...
                                             'browser': ('browser', []),
                                             'appcategory': ('appcategory', ['category'])},
                                  'quarantine': {'managementID': ('managementID', []),
                                                 'nsdeviceuid': ('nsdeviceuid', []),
                                                 'alert_type': ('alert_type', []),
                                                 'dstip': ('dstip', ['ip', 'ipv4']),
                                                 'file_type': ('file_type', []),
                                                 ...
                                                 'browser': ('browser', []),
                                                 'appcategory': ('appcategory', ['category'])},
                                  'security assessment': {'managementID': ('managementID', []),
                                                          'nsdeviceuid': ('nsdeviceuid', []),
                                                          'alert_type': ('alert_type', []),
                                                          'region_id': ('region_id', []),
                                  ...
                                  'watchlist': {'managementID': ('managementID', []),
                                                'nsdeviceuid': ('nsdeviceuid', []),
                                                'alert_type': ('alert_type', []),
                                                'app': ('app', []),
                                                'object_id': ('object_id', []),
    ...
    def _update_url_helper(self, action_result):
        ...
        try:
            ...
            if self._v2_api_key:
                ...
                request_status, _ = self._make_rest_call(endpoint=(('{0}/replace').format(url_list_endpoint)), action_result=action_result, data=data, method='patch')
    ...
    def _handle_remove_url_list(self, param):
        ...
        status, remove_msg = phantom_rules.delete_from_list(
            ...
            value=param[NETSKOPE_PARAM_URL],
            remove_all=True,
            remove_row=True)
    ...
    def _get_url_list_id(self, action_result):
        ...
        request_status, request_response = self._make_rest_call(endpoint=NETSKOPE_V2_URL_LIST_ENDPOINT, action_result=action_result, params=request_params)
        ...
        return action_result.set_status(phantom.APP_ERROR,
                                        'Error: A URL List object with the name provided does not exist on the Netskope. Create one first.'), None
    ...
    def _handle_scim_user_to_group(self, param):
        ...
        try:
            data = {
                'Operations': [{
                    'op': param.get('action', 'add'),
                    'path': 'members',
                    'value': [{'value': param[NETSKOPE_PARAM_USER]}]
                }],
                'schemas': ['urn:ietf:params:scim:api:messages:2.0:PatchOp']
            }
            self._log.info(('action=make_scim_rest_call params={0}').format(data))
            request_status, request_response = self._make_scim_rest_call(endpoint=('{0}/{1}').format(NETSKOPE_SCIM_GROUPS_ENDPOINT, param[NETSKOPE_PARAM_GROUP]),
    ...
    def _handle_scim_create_group(self, param):
        ...
        action_result = self.add_action_result(ActionResult(dict(param)))
        try:
            data = {'displayName': param[NETSKOPE_PARAM_GROUP],
    ...
    def _handle_remove_file_list(self, param):
        ...
        status, remove_msg = phantom_rules.delete_from_list(list_name=self._file_list, value=param[NETSKOPE_PARAM_HASH],
